...
class ValueAdditionPage extends StatefulWidget {
  const ValueAdditionPage({Key? key}) : super(key: key);

  @override
  _ValueAdditionPage createState() => _ValueAdditionPage();
}

class _ValueAdditionPage extends State<ValueAdditionPage> {
  List establishment = [];
  List searchList = [];
  bool loading = true;
    ...
    return Stack(children: [
      Scaffold(
        appBar: AppBar(
            title: Row(children: [
              Flexible(
                child: Text("Value Addition Opportunities",
                    maxLines: 15,
                    style: TextStyle(
                        color: Colors.white,
                        fontSize: 18,
                              ...
                              child: Container(
                                ...
                                Container(
                                  ...
                                  child: this.internetCheck == false
                                      ? Image.asset(
                                          "assets/images/ic_value_foreground.png",
                                          width: 100,
                                        )
                                      : Image.network(
  ...
  Future fetchFarmings() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        prefs.setString("value_addition", res.body);
        List<dynamic> data = map["data"];

        //filter before returning data.
        filteredData =
            data.where((e) => e["category"] == "ValueAddition").toList();
        loading = false;
      }
    } catch (e) {
      loading = false;
  ...
  Future<List> getData() async {
    ...
    loading = true;
    try {
      if (prefs.getString("value_addition") == null) {
        final ioc = new HttpClient();
        ioc.badCertificateCallback =
            (X509Certificate cert, String host, int port) => true;
        final http = new IOClient(ioc);
        ...
        String storedData = prefs.getString("value_addition").toString();
        Map<String, dynamic> map = json.decode(storedData);
        List<dynamic> data = map["data"];

        //filter before returning data.
        filteredData = data
            .where((e) => e["category"].toString() == "ValueAddition")
            .toList();
        setState(() {
          loading = false;
        });
